...
/**
 * Estados posibles de invoices.status en la BD compartida.
 * 'pending' | 'paid' los gestiona Publiko; 'draft' | 'sent' | 'overdue' | 'cancelled'
 * los introdujo Labs OS (migración 0023). Todo código que filtre o muestre facturas
 * debe tolerar el set completo.
 */
...
export interface Database {
  public: {
    Tables: {
      ...
      invoices: {
        Row: { id: string; client_id: string; invoice_number: string; amount: number; invoice_type: string; description: string | null; period_start: string | null; period_end: string | null; status: InvoiceStatus; due_date: string | null; issued_date: string | null; paid_at: string | null; payment_method: string | null; pdf_url: string | null; lines: Json; subtotal: number | null; tax_amount: number | null; irpf_amount: number | null; notes: string | null; sent_at: string | null; created_by: string | null; organization_id: string; workspace_id: string | null; project_id: string | null; created_at: string; updated_at: string }
