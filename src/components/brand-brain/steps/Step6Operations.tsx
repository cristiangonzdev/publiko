...
export function Step6Operations({ data, update, preset }: StepProps) {
  const schedule = (data.schedule as Record<string, string>) ?? {}
  const booking = (data.booking as Record<string, unknown>) ?? {}
  const goals = (data.social_goals as Record<string, unknown>) ?? {}

  return (
    <div className="space-y-5">
      <h2 className="font-serif text-xl text-ink-900">6. Operaciones y objetivos</h2>
      <p className="text-sm text-ink-500">Horarios, contacto y metas en redes. Esta info evita publicar cuando están cerrados.</p>
      ...
      <FieldRow>
        <TextInput label="Email de contacto" value={booking.email as string ?? ''} onChange={(v) => update('booking', { ...booking, email: v })} />
        <TextInput label="CTA principal" placeholder={preset.bookingCtaPlaceholder} value={booking.booking_cta as string ?? ''} onChange={(v) => update('booking', { ...booking, booking_cta: v })} />
      </FieldRow>

      <h3 className="text-sm font-semibold text-ink-700 pt-2">Objetivos en redes</h3>
      <TextInput label="Objetivo principal" placeholder={preset.primaryGoalPlaceholder} value={goals.primary as string ?? ''} onChange={(v) => update('social_goals', { ...goals, primary: v })} />
      ...
      <TextArea
        ...
        placeholder="Info relevante que la IA debe conocer y no encaja en otra sección…"
        value={data.important_context as string ?? ''}
        onChange={(v) => update('important_context', v)}
      />
      <TextArea
        label="Notas internas (no se muestran al cliente)"
        value={data.client_notes as string ?? ''}
